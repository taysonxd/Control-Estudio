                ...
                <div class="form-group row"> <!-- Full Name -->
                    ...
                    <div class="col-md-4">
                        ...
                        @forelse($trayectos as $trayecto)
                            ...
                            @empty
                            <option>Asigne trayecto a la carrera.</option>
                        @endforelse
                        </select>
                    </div>
                </div>
...
$(document).ready(function(){
    ...
    $("#id_trayecto").on("change", function(){

        var datos = {
            anio : "{!! $anio !!}",
            id_carrera : "{!! $carrera->id !!}",
            trayecto : $("#id_trayecto option:selected").text()
        };

        $.ajax({
            async:true,
            url: "{{ route('carga-preladoras') }}",
            cache:false,
            dataType:"json",
            type:  'POST',
            data:  datos,
            ...
            success:function (response) {
                ...
                if(response.length != undefined){
                    ...
                    $(response).each(function(key, value){
                        ...
                        $('#prelaciones').append('<option value='+value.id+'>'+(value.desc_asignatura == 'default'? value.materia:value.desc_asignatura)+'</option>');
                    });
                    //console.log(response);
                }
            },
